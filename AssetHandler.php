...
class AssetHandler
{
   public static function processAssets($html, $template)
   {
      ...
      $handledTemplates = ['thread_view', 'xfmg_media_view', 'xfrm_resource_view', 'dbtech_ecommerce_product_view'];

      // Here we can add additional templates to lazy-fetch some images
      $priorityTemplates = ['page_view', 'EWRmedio_medias_list'];

      if (isset($options->nlXFOptimizePriorityPreloadAddTemplates))
      {
         $additionalTemplates = explode("\n", $options->nlXFOptimizePriorityPreloadAddTemplates);
         $priorityTemplates = array_unique(array_merge($priorityTemplates, $additionalTemplates) , SORT_REGULAR);
      }

      //var_dump($template);
      // Setup directives
      $isPreconnect = $options->nlXFOptimizeAutomaticPreconnect;

      $isPriorityPreload = $options->nlXFOptimizePreloadMode == 'priority' && in_array($template, $priorityTemplates) && !in_array($template, $handledTemplates);
      $isManualPreload = $options->nlXFOptimizePreloadMode == 'manual' || $isPriorityPreload;
	   
      // We only test for two conditions here, as Posts mode is handled separately
      $isPreload = $isManualPreload || $isPriorityPreload;
      $isAutoPreload = $options->nlXFOptimizePreloadMode == 'auto';
	 ...
	 'productIcons'
      ...
      if ($isPreload)
      {
         ...
         // Limit
         $limit = $options->nlXFOptimizePreloadLimitPerContent['enabled'] ? $options->nlXFOptimizePreloadLimitPerContent['attachments'] : null;
   ...
   protected static function getAssetUrls($elements, string $from, string $siteHostRootHost, array $disallowed = [])
   {
      ...
      foreach ($elements as $item)
      {
         ...
         if ($from == 'bg-style' || ($from == 'data-preload' && $item->nodeName !== 'img' && $item->hasAttribute("style")) || ($from == 'priority' && $item->nodeName !== 'img' && $item->hasAttribute("style")))
         {
         ...
         // Process image tags
         if (($from == 'img' && $item->hasAttribute("src")) || ($from == 'data-preload' && $item->nodeName == 'img' && $item->hasAttribute("src")) || ($from == 'priority' && $item->nodeName == 'img' && $item->hasAttribute("src")))
         {
...
class PriorityPreload
{
   public static function processContent($content, string $contentType, $limit)
   {
      //throw new \LogicException('PriorityPreload loaded.');
      // Setup vars
      $priorityPreloadUrls = [];
      $priorityPreloadImages = [];

      // Get full attachments from description
      $attachItems = [];
      $attachUrls = [];

      preg_match_all('/(.*?\s?)?\[attach.*?\](.*?)\[\/attach\]/ims', $content, $attachMatches, PREG_PATTERN_ORDER);

      $length = 0;
      $currentLength = 0;
      $attachLengths = [];

      if (!empty($attachMatches))
      {
         $attachIds = [];

         // Get distance from beginning of message
         foreach ($attachMatches[1] as $match)
         {
            $length = strlen($match);
            $currentLength = $length + $currentLength;
            $attachLengths[] = $currentLength;
         }

         // Get attachment IDs
         foreach ($attachMatches[2] as $match)
         {
            $attachIds[] = $match;
         }
         ...
         $attachments = $finder->where('content_type', $contentType)->where('attachment_id', $attachIds)->fetch();

         // Get the attachment image URLs
         foreach ($attachments as $attachment)
         {
            if (!$attachment->getDirectUrl())
            {
               $attachUrls[] = 'Failed to get URL';
            }
            else
            {
               $attachUrls[] = $attachment->getDirectUrl();
            }
         }

         $attachCombined = [];
         foreach ($attachUrls as $key => $value)
         {
            $attachCombined[$key] = ['url' => $value, 'length' => $attachLengths[$key]];
         }
      }

      // Get img tags from description
      $imgUrls = [];
      preg_match_all('/(.*?\s?)?\[img.*?\](.*?)\[\/img\]/ims', $content, $imgMatches, PREG_PATTERN_ORDER);
      ...
      if (!empty($imgMatches))
      {
         ...
         // Get distance from beginning of message
         foreach ($imgMatches[1] as $match)
         {
            $length = strlen($match);
            $currentLength = $length + $currentLength;
            $imgLengths[] = $currentLength;
         }

         foreach ($imgMatches[2] as $match)
         {
            $imgUrls[] = $match;
         }
         ...
         {
            $imgCombined[$key] = ['url' => $value, 'length' => $imgLengths[$key]];
         }
      }

      // Combined images from attach and img tags
      $merged = array_unique(array_merge($attachCombined, $imgCombined) , SORT_REGULAR);

      // Sort by length from start
      ...
      {
         $priorityPreloadImages[] = $item['url'];
      }

      return $priorityPreloadImages;
   }
}
